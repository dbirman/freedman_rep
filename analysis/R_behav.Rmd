...
```{r}
files = dir('C:\\Users\\Dan\\proj\\freedman_rep\\dircsv\\')

adata_dir = data.frame();
for (fi in 1:length(files)) {
  fname = files[fi]
  data = read.csv(paste('C:\\Users\\Dan\\proj\\freedman_rep\\dircsv\\',fname,sep=''))
  data = data %>%
    mutate(wid=fi)
  adata_dir = rbind(adata_dir,data)
}
files = dir('C:\\Users\\Dan\\proj\\freedman_rep\\catcsv\\')

adata_cat = data.frame();
for (fi in 1:length(files)) {
  fname = files[fi]
  data = read.csv(paste('C:\\Users\\Dan\\proj\\freedman_rep\\catcsv\\',fname,sep=''))
  data = data %>%
    mutate(wid=fi)
  adata_cat = rbind(adata_cat,data)
}

files = dir('C:\\Users\\Dan\\proj\\freedman_rep\\longcsv\\')

adata_long = data.frame();
for (fi in 1:length(files)) {
  fname = files[fi]
  data = read.csv(paste('C:\\Users\\Dan\\proj\\freedman_rep\\longcsv\\',fname,sep=''))
  data = data %>%
    mutate(wid=fi)
...
adata_long$wid = adata_long$wid + max(adata_cat$wid)
...
ggplot() + 
  ...
  xlab('Trial #') +
  coord_cartesian(ylim=c(.45,1.05)) +
